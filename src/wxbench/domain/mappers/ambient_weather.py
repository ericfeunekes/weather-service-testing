...
def _map_ambient_record(
    ...
    *,
    ...
    observed_at = _parse_observed_at(record.get("dateutc"))

    temperature_f = _to_optional_float(record.get("tempf"))
    if temperature_f is None:
        temperature_f = _to_optional_float(record.get("tempOut"))
    if temperature_f is None:
        temperature_c_metric = _to_optional_float(record.get("tempc"))
    else:
        temperature_c_metric = None

    temperature_in_f = _to_optional_float(record.get("tempinf"))
    if temperature_in_f is None:
        temperature_in_f = _to_optional_float(record.get("tempIn"))

    feels_like_f = _to_optional_float(record.get("feelsLike"))
    feels_like_in_f = _to_optional_float(record.get("feelsLikein"))

    dewpoint_f = _to_optional_float(record.get("dewPoint"))
    if dewpoint_f is None:
        dewpoint_f = _to_optional_float(record.get("dewptf"))
    if dewpoint_f is None:
        dewpoint_c_metric = _to_optional_float(record.get("dewpt"))
    else:
        dewpoint_c_metric = None

    dewpoint_in_f = _to_optional_float(record.get("dewPointin"))
    if dewpoint_in_f is None:
        dewpoint_in_f = _to_optional_float(record.get("dewptin"))

    wind_speed_mph = _to_optional_float(record.get("windspeedmph"))
    if wind_speed_mph is None:
        wind_speed_mph = _to_optional_float(record.get("windSpeed"))

    wind_gust_mph = _to_optional_float(record.get("windgustmph"))
    max_daily_gust_mph = _to_optional_float(record.get("maxdailygust"))

    pressure_inhg = _to_optional_float(record.get("baromrelin"))
    if pressure_inhg is None:
        pressure_inhg = _to_optional_float(record.get("baromabsin"))
    if pressure_inhg is None:
        pressure_inhg = _to_optional_float(record.get("barometer"))

    pressure_abs_inhg = _to_optional_float(record.get("baromabsin"))

    precipitation_in = _to_optional_float(record.get("hourlyrainin"))
    if precipitation_in is None:
        precipitation_in = _to_optional_float(record.get("hourlyrain"))

    precipitation_daily_in = _to_optional_float(record.get("dailyrainin"))
    precipitation_weekly_in = _to_optional_float(record.get("weeklyrainin"))
    precipitation_monthly_in = _to_optional_float(record.get("monthlyrainin"))
    precipitation_yearly_in = _to_optional_float(record.get("yearlyrainin"))
    precipitation_event_in = _to_optional_float(record.get("eventrainin"))
    battery_in = _to_optional_float(record.get("battin"))
    battery_out = _to_optional_float(record.get("battout"))
    ...
    return Observation(
        ...
        wind_direction_deg=_to_optional_int(record.get("winddir")),
        wind_direction_avg_10m_deg=_to_optional_int(record.get("winddir_avg10m")),
        pressure_kpa=_inHg_to_kpa(pressure_inhg),
        pressure_absolute_kpa=_inHg_to_kpa(pressure_abs_inhg),
        relative_humidity=_to_optional_float(record.get("humidity")),
        relative_humidity_in=_to_optional_float(record.get("humidityin")),
        visibility_km=None,
        condition=None,
        ...
        uv_index=_to_optional_float(record.get("uv")),
        solar_radiation_wm2=_to_optional_float(record.get("solarradiation")),
...
def map_ambient_weather_observation(
    ...
    return _map_ambient_record(last_data, location=location, station=station, provider=provider)


def map_ambient_weather_history(
    payload: Sequence[Mapping[str, Any]],
    *,
    location: Location,
    station: Optional[str],
    provider: str = "ambient_weather",
) -> list[Observation]:
    if not payload:
        return []
    ...
    for record in payload:
        observations.append(_map_ambient_record(record, location=location, station=station, provider=provider))

    observations.sort(key=lambda entry: entry.observed_at)
    return observations


__all__ = ["map_ambient_weather_observation", "map_ambient_weather_history"]
